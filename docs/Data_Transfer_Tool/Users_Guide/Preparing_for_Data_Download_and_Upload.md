...
A UUID can be used for data submission with the Data Transfer Tool. A UUID for data uploads can be obtained from the Submission Portal or from the API GraphQL endpoint.  In the Submission Portal the UUID for the data files are found in the Manifest file under the IDs column or in the browse tab page under Submittable Data Files section.  Opening up the files details page by clicking on the Submitter ID of the file will display a Summary page which contains the UUID of the file.
