...
|---|---|
|This block will go out of table|[BEDPE file format](https://bedtools.readthedocs.io/en/latest/content/general-usage.html#bedpe-format), (**b**rowser **e**xtensible **d**ata **p**aired-**e**nd) is designed to concisely describe disjoint genome features, such as structural variations or paired-end sequence alignments (see [this]). It's an enhanced version of the [BED format](http://genome.ucsc.edu/FAQ/FAQformat#format1), since BED does not allow inter-chromosomal feature definitions. In addition, BED only has one strand field, which is insufficient for paired-end sequence alignments, especially when studying structural variation. The BEDPE format is described below.|
...
|This block will go out of table|In addition to the above fields, bedtools allows one to add as many additional user-defined fields to the normal, 10-column BEDPE format as necessary. These columns are merely “passed through” pairToBed and pairToPair and are not part of any analysis. One would use these additional columns to add extra information (e.g., edit distance for each end of an alignment, or “deletion”, “inversion”, etc.) to each BEDPE feature.|
<br>
...
|Column Header|Description|
|---|---|
|This block will go oput of table|AscatNGS, originally developed by [Raine *et al* (2016)]( https://doi.org/10.1002/cpbi.17) ([GitHub page](https://github.com/cancerit)), indicates the DNA copy number changes affecting a tumor genome when comparing to a matched normal sample. The data may be available in tab separated values (.TSV) or plain text file (.TXT)|
|GDC Aliquot |The GDC ID for the aliquot collected from the sample (copy number segment files only)|
|Gene ID |The gene ENSMBL ID (copy number variant only)
...
|Copy Number|The weighted median of the strand copy numbers |
...
|This block will go out of table|Based on comments from Zhenyu, there is a weighted midpoint for copy numbers that has to be taken into consideration (for major/minor & max min copy numbers). We may need to include a brief introductory about it.|
