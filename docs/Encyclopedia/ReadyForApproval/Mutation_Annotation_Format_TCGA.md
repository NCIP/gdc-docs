...
-  **exclude** all other types of mutation calls (i.e. non-somatic calls (validated or not), unvalidated somatic call that are not in coding region or splice sites, and dbSNP sites that are not annotated as somatic in dbSNP, COMIC, or OMIM)
