...
## Overview ##
Data from the Affymetrix SNP 6.0 platform is used by the GDC to produce harmonized Copy Number Variation files<sup>2</sup>. Hybridization levels associated with Affymetrix SNP 6.0 probes are used to determine discrete chromosomal regions that show signs of duplication. A comprehensive documentation of the GDC Copy Number Variation pipeline can be found at the GDC Documentation Website<sup>1</sup>. A list of all probes and the chromosomal regions that associate with them (GRCh38 coordinates) can be found in the GDC Reference File webpage<sup>2</sup>.
