...
The final tab of filters are for specific mutations.  Users can filter by:

* __Mutation ID__ - Unique ID for that mutation.  Users can use the following:
...
* __SSM Caller__ - The variant caller used to identify the mutation
...
At the top of this tab is a bar graph of the 20 most frequent mutations in the cohort as a bar graph that indicates the percentage of cases that are affected by each mutation. Hovering over each bar in the plot will show information about the number of cases affected.  It also includes a survival plot of all the cases within the specified exploration page filters.
