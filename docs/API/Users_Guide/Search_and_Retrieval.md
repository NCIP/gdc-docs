...
The first step is to construct a JSON object that contains the query, and [percent-encode](http://text-rescue.com/string-escape/percent-url-encoding-tool.html) it. Here we look for 15 files using their UUIDs:
...
```
```Shell
curl --header POST -d @Payload 'https://gdc-api.nci.nih.gov/files' > File_metadata
```
```File_metadata
