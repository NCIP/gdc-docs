...
The first step is to construct a JSON query object for the `filters` parameter, and [percent-encode](http://text-rescue.com/string-escape/percent-url-encoding-tool.html) it. Here we look for 15 files using their UUIDs:
...
Now we construct the remainder of the query, that specifies the format of the response, including `fields`, `format`, and `size`:

	fields=file_id,file_name,cases.submitter_id,cases.case_id,data_category,data_type,cases.samples.tumor_descriptor,cases.samples.tissue_type,cases.samples.sample_type,cases.samples.submitter_id,cases.samples.sample_id&format=tsv&size=100

Putting it all together we get our HTTP POST payload, which we submit to the GDC API using curl to get a TSV file with the file metadata we requested:
