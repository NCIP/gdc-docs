...
* Examples provided for BAM slicing functionality are intended for use with GDC harmonized data (i.e. BAM files available in the GDC Data Portal). Slicing of unharmonized BAM files (i.e. BAM files in the GDC Legacy Portal) is not supported.  however, users should be aware of inconsistencies in the choice of reference genome used for alignment of unharmonized BAMs (e.g. GRCh38 vs GRCh37) and inconsistent chromosome identifiers (e.g. "chr1" vs "1"). Slicing using GENCODE/HGNC gene names is not supported for unharmonized files. 
